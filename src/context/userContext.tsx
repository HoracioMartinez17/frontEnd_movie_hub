...
import {
  ...
  , updateMoviesUser, deleteMoviesUser, putUpdateUsers,deleteUserById
...
interface UserContextType {
  ...
  updateUsersData: (userId: string, getAccessTokenSilently: any, userUpdate: userUpdate) => void
  deleteUsersData: (userId: string, getAccessTokenSilently: any) => Promise<Response>;
}
...
export const UserProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const updateUsersData = async (userId: string, getAccessTokenSilently: any, userUpdate: userUpdate) => {
    try {
      // Update user data on the server
      await putUpdateUsers(userId, getAccessTokenSilently, userUpdate);
      // No response to update state with
    } catch (error) {
      console.error('Error updating user:', error);
    }
  }
  const deleteUsersData = async (userId: string, getAccessTokenSilently: any): Promise<Response> => {
    try {
  ...
  return (
    <UserContext.Provider value={{
      ...
      fetchUserMoviesByGenres, moviesSave, moviesUpdate, movieDelete, updateUsersData,
      deleteUsersData
    }}>
      {children}
